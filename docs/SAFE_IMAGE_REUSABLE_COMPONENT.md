...
## This repository (vanilla HTML / no Next.js)

Remote images use **`data-dynamic-img`** plus optional **`data-fallback-img`** (keys in `js/data.js`). On **`error`**, `applyDynamicImages()` in `js/ui.js` swaps the `src` to the fallback URL — same idea as `SafeImage`, without React or `/_next/image`.
